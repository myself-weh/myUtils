...
public class StringUtil {
	...
	*/
	...
	// 方法3-2：返回length个中文汉字字符串，字符集必须在GB2312(相当于中文简体)范围内，例如“中呀被”(5分)
	...
	//方法1：随机产生n个中文字符串 (8分)
		public static String getZHString(int n){
			...
			for (int i = 0; i< n; i++) {
				...
				int lowCode;
				Random random = new Random();
				higCode = (176+Math.abs(random.nextInt(39)));
				lowCode = (161+Math.abs(random.nextInt(93)));
				byte[] b = new byte[2];
				b[0] = (Integer.valueOf(higCode)).byteValue();
		...
		public static String getZHName(){
			String xs = "";
			String ming = "";
			String file = StreamUtil.getFile("C:\\Users\\Mr.Wu\\Desktop\\百家姓.txt");
			String[] xing = file.split(",");
			int index;
			xs = xing[RandNumUtil.random(0, xing.length)].replace("\"", "");
			ming = getZHString(2);
			return xs+ming;
